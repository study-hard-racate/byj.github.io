...
(function (global) {
  ...
  /* 生成"好看"的刻度：步长落在 1/2/2.5/5 × 10^n 上，避免出现 1000 一档的粗刻度 */
  ...
  function LineChart(container, opt) {
    ...
    series.forEach(s => {
      const data = s.data || [];
      let d = "", area = "", started = false, lastIdx = -1;
      data.forEach((v, i) => {
        const isGap = v === null || v === undefined || (opt.treatZeroAsGap && Number(v) === 0);
        if (isGap) { started = false; return; }
        const x = X(i), y = Y(Number(v));
        d += (started ? " L" : " M") + x.toFixed(1) + " " + y.toFixed(1);
        if (!started) { area += (lastIdx === -1 ? " M" : " M") + x.toFixed(1) + " " + (pad.t + ih) + " L"; }
        area += x.toFixed(1) + " " + y.toFixed(1) + " L";
        started = true; lastIdx = i;
      });
      if (s.fill !== false && d) {
        const lastGapFix = area.replace(/ L$/, "");
        el("path", {
          d: lastGapFix + " L" + X(lastIdx).toFixed(1) + " " + (pad.t + ih) + " Z",
