...
/** 解析文件（按扩展名/内容分派：.xlsx 走 Excel 解析，.xls 提示转换，其余按 CSV） */
async function parseBillFile(bytes, filename = "", manualSource = "") {
  // file.arrayBuffer() 给的是 ArrayBuffer，zip/xlsx 解析按字节下标读，需 Uint8Array
  if (bytes instanceof ArrayBuffer) bytes = new Uint8Array(bytes);
  const lower = String(filename).toLowerCase();
  // 旧版二进制 .xls（OLE2 文件头）——浏览器无法解析，给明确指引
  const isOle2 = bytes.length >= 8 &&
    bytes[0] === 0xD0 && bytes[1] === 0xCF && bytes[2] === 0x11 && bytes[3] === 0xE0 &&
    bytes[4] === 0xA1 && bytes[5] === 0xB1 && bytes[6] === 0x1A && bytes[7] === 0xE1;
  if (lower.endsWith(".xls") || isOle2) {
    return {
      records: [],
      meta: { source: manualSource || "xls", total_rows: 0, skipped: 0,
              warnings: ["这是旧版 .xls 格式，浏览器无法直接解析。请用 Excel/WPS 打开后「另存为 → .xlsx」再上传（微信导出的对账文件是 .xlsx，可直接上传）"] },
    };
  }
      ...
      return {
        ...
        meta: { source: manualSource || "xlsx", total_rows: 0, skipped: 0,
                warnings: ["Excel 解析失败：" + e.message +
                  "。如果是 .xls 请另存为 .xlsx；仍失败的话把文件「另存为 CSV」再上传"] },
      };
    }
  }
