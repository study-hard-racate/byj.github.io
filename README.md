...
- 💰 账单管理：导入支付宝 / 微信官方导出的 CSV / Excel，自动分类，图表分析
...
3. **打卡**：健康页每天 20 秒（表单始终可用，即使还没有任何记录）。
4. **备份**：设置页导出 JSON 备份，换设备时导入恢复；超过 14 天没备份，总览页会提醒。
5. **提醒**：设置页可调打卡提醒时间 / 账单导入提醒日，并开启系统通知；打开或切回本站时自动检查（纯前端无法后台定时）。

## 测试与开发约定

- 单元测试：`node tools/run-tests.js`（解析器 / Excel / 健壮性 / 数据层共 63 项，见 `tools/`），也可直接 `npm test`
...
├── sw.js                  # Service Worker（离线缓存，发版时递增版本）
├── tools/                 # 测试与开发工具（run-tests / TESTING.md / 数据生成器）
