...
1. **导入账单**：支付宝 / 微信 App 里导出「用于个人对账」的 CSV（官方 zip 带密码，先解压成 CSV），
   在「导入规则」页上传 → 预览 → 确认导入。GBK / UTF-8 编码自动识别，按内容指纹自动去重。
...
- 纯前端方案的限制：提醒只在**打开网站时**生效；zip 需先解压成 CSV。
